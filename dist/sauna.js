// ── Config ──────────────────────────────────────────────────────────────────
// Edit these values to update the display without touching anything else.
...
const STAFF_MESSAGES = [{
  ...
  signature: "Marlene & Johann"
}, {
  label: "Gut zu wissen",
  message: "Bitte denken Sie daran, ausreichend Wasser zu trinken — an unserem Wasserspender stehen frische Aufgüsse bereit.",
  signature: "Ihr Saunahuus-Team"
}, {
  label: "Herzlich willkommen",
  message: "Schön, dass Sie heute bei uns sind. Entspannen Sie sich in aller Ruhe — wir sind für Sie da.",
  signature: "Das Saunahuus-Team"
}];

// Fallback shown while the first fetch is in flight or if the network is unavailable.
...
// Maps WMO weather-code → { condition, conditionDe }
// Open-Meteo uses WMO standard codes: https://open-meteo.com/en/docs#weathervariables
function wmoToCondition(code) {
  ...
  if (code >= 80 && code <= 82) return {
    condition: "rainy",
    conditionDe: "Regenschauer"
  };
  if (code >= 85 && code <= 86) return {
    condition: "rainy",
    conditionDe: "Schneeschauer"
  };
  if (code >= 95) return {
    condition: "rainy",
    conditionDe: "Gewitter"
  };
  return {
    condition: "cloudy",
    conditionDe: "Bewölkt"
...
function useWeather(refreshKey) {
  const [weather, setWeather] = React.useState(WEATHER_FALLBACK);
  React.useEffect(() => {
    const url = `https://api.open-meteo.com/v1/forecast` + `?latitude=${CONFIG.weatherLat}&longitude=${CONFIG.weatherLon}` + `&current=temperature_2m,weather_code` + `&daily=temperature_2m_max,temperature_2m_min` + `&timezone=auto&forecast_days=1`;
    async function fetchWeather() {
      try {
        ...
        const {
          condition,
          conditionDe
        } = wmoToCondition(data.current.weather_code);
        setWeather({
        ...
        // keep whatever was last shown (or the fallback on first load)
      }
    }
    fetchWeather();
...
function useNews(refreshKey) {
  const [news, setNews] = React.useState(null);
  React.useEffect(() => {
    if (!CONFIG.newsApiUrl) return;
    async function fetchNews() {
      try {
        ...
        const items = data[0].data ? data[0].data : [];
        if (items.length === 0) {
          setNews(null);
          return;
        }
        const sorted = [...items].sort((a, b) => new Date(b.lastPublished) - new Date(a.lastPublished)).slice(0, 5);
        setNews(sorted);
      } catch {
        setNews(null);
      }
...
function useLoadLevel(refreshKey) {
  const [level, setLevel] = React.useState(() => {
    const base = LOAD_LEVELS[CONFIG.load] || LOAD_LEVELS.gruen;
    return {
      status: CONFIG.load,
      ...base
    };
  ...
  React.useEffect(() => {
    if (!CONFIG.loadApiUrl) return;
    async function fetchLoad() {
      try {
        const res = await fetch(CONFIG.loadApiUrl);
        if (!res.ok) throw new Error("fetch failed");
        const data = await res.json();
        const {
          status,
        ...
        const s = status === 'status-rot' ? 'rot' : status === 'status-gelb' ? 'gelb' : 'gruen';
        const currentStatus = items && items.length > 0 ? items[0] : {};
        const label = currentStatus.label || LOAD_LEVELS[s]?.label || "Unbekannt";
        const head = currentStatus.head || LOAD_LEVELS[s]?.head || "";
        const body = currentStatus.body || LOAD_LEVELS[s]?.body || "";
        const colors = LOAD_LEVELS[s] || LOAD_LEVELS.gruen;
        setLevel({
          status: s,
          label,
          head,
    ...
    fetchLoad();
    const id = setInterval(fetchLoad, 1 * 60 * 1000); // refresh every 1 min
    return () => clearInterval(id);
  }, [refreshKey]);
  return level;
}
  ...
  React.useEffect(() => {
    ...
    async function fetchOpening() {
      try {
        ...
        // JS getDay(): 0=Sun,1=Mon…6=Sat → ISO weekday: Mon=1…Sun=7
        const jsDay = new Date().getDay();
        const isoDay = jsDay === 0 ? 7 : jsDay;
        const match = items.find(item => {
          const days = String(item["tage-der-woche"] || "").split(",").map(d => parseInt(d.trim(), 10)).filter(d => !isNaN(d));
        ...
        // Parse "HH:MM - HH:MM Uhr" → { openH, openM, closeH, closeM }
        const m = String(match.zeitraum || "").match(/(\d{1,2}):(\d{2})\s*[-–]\s*(\d{1,2}):(\d{2})/);
        setOpening({
          closed: false,
          zeitraum: match.zeitraum,
          details: match.details || "",
          openH: m ? parseInt(m[1], 10) : null,
          openM: m ? parseInt(m[2], 10) : 0,
          closeH: m ? parseInt(m[3], 10) : 22,
          closeM: m ? parseInt(m[4], 10) : 0
        });
      } catch {
        setOpening(null);
      }
...
function useStaffMessages(refreshKey) {
  const initial = !CONFIG.staffMessagesApiUrl && STAFF_MESSAGES.length > 0 ? STAFF_MESSAGES : null;
  const [messages, setMessages] = React.useState(initial);
  React.useEffect(() => {
    if (!CONFIG.staffMessagesApiUrl) return;
    async function fetchMessages() {
      try {
        const res = await fetch(CONFIG.staffMessagesApiUrl);
        if (!res.ok) throw new Error("fetch failed");
        const data = await res.json();
        const items = Array.isArray(data) ? data : data.data || [];
        setMessages(items.length > 0 ? items : null);
      } catch {
        setMessages(null);
      }
    }
    fetchMessages();
    const id = setInterval(fetchMessages, 5 * 60 * 1000); // refresh every 5 min
    return () => clearInterval(id);
  }, [refreshKey]);
  return messages;
}
...
function PlaceholderPhoto({
  ...
  const bg = tone === "cool" ? ["#E7ECEF", "#DBE2E7"] : tone === "wood" ? ["#E8DCC8", "#DECBAE"] : ["#EFE5DB", "#E4D6C6"];
...
function TopBar({
  ...
  }, CONFIG.logoUrl ? /*#__PURE__*/React.createElement("img", {
    src: CONFIG.logoUrl,
    alt: "Logo",
    style: {
      height: 40,
  ...
  }, "SAUNAHUUS"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontFamily: "'Poppins', sans-serif",
      fontSize: 12,
      fontWeight: 500,
  ...
  }, "Status · Aushang")), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      alignItems: "center",
      gap: 18
...
function ClockBlock() {
  ...
  }, /*#__PURE__*/React.createElement(SectionLabel, null, "Uhrzeit"), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
    style: {
      fontFamily: "'Clash Display Variable', sans-serif",
      fontWeight: 300,
      fontSize: 96,
...
function WeatherGlyph({
  ...
  const stroke = "#2b2720";
  if (kind === "sunny") return /*#__PURE__*/React.createElement("svg", {
    width: size,
    height: size,
    viewBox: "0 0 64 64",
    fill: "none"
  ...
  if (kind === "cloudy") return /*#__PURE__*/React.createElement("svg", {
    width: size,
    height: size,
    viewBox: "0 0 64 64",
    fill: "none"
  ...
  if (kind === "rainy") return /*#__PURE__*/React.createElement("svg", {
    width: size,
    height: size,
    viewBox: "0 0 64 64",
    fill: "none"
...
function WeatherBlock({
  ...
  }, conditionDe), /*#__PURE__*/React.createElement("span", {
    style: {
      color: "rgba(40,35,28,0.55)",
      fontVariantNumeric: "tabular-nums"
    }
...
function Thermometer({
  ...
  // Mercury fill level per status (0 = empty bottom, 1 = full top)
  const fillRatio = {
    gruen: 0.22,
    gelb: 0.58,
    rot: 0.92
  ...
  // SVG geometry constants (viewBox "0 0 80 290")
  const cx = 40;
  const tubeTopY = 16;
  const tubeBottomY = 224; // where tube meets bulb
  const tubeH = tubeBottomY - tubeTopY; // 208
  const tubeInnerW = 16;
  const tubeInnerX = cx - tubeInnerW / 2; // 32
  const bulbCY = 256;
  const bulbOuterR = 27;
  const bulbInnerR = 20;
  const mercuryTopY = tubeBottomY - fillRatio * tubeH;
  return /*#__PURE__*/React.createElement("div", {
    style: {
      padding: "20px 22px",
      background: "#1c1a16",
    ...
    clipPath: "url(#thermo-tube-clip)"
  }, /*#__PURE__*/React.createElement("rect", {
    style: {
      x: tubeInnerX,
      y: mercuryTopY,
      width: tubeInnerW,
      height: tubeBottomY - mercuryTopY,
      fill: color,
      transition: "y 0.9s cubic-bezier(0.4,0,0.2,1), height 0.9s cubic-bezier(0.4,0,0.2,1), fill 0.6s ease"
    }
  })), [0.25, 0.5, 0.75].map(t => {
    const y = tubeBottomY - t * tubeH;
    return /*#__PURE__*/React.createElement("line", {
      key: t,
      x1: cx + 9,
      y1: y,
...
function LoadHero({
  ...
  }, level.head), /*#__PURE__*/React.createElement("div", {
    style: {
      marginTop: 12,
      fontFamily: "'Poppins', sans-serif",
      fontSize: 24,
...
function NewsSlideshow({
  ...
  const [dir, setDir] = React.useState(1); // 1 = forward (right→left), -1 = backward

  // Reset to first slide whenever the news data changes.
  React.useEffect(() => {
    setIdx(0);
    setProg(0);
    setDir(1);
  ...
  const safeIdx = idx % news.length;
  const item = news[safeIdx];
  const slideAnim = dir >= 0 ? "slideInFromRight 0.5s cubic-bezier(0.22,1,0.36,1) both" : "slideInFromLeft  0.5s cubic-bezier(0.22,1,0.36,1) both";
  return /*#__PURE__*/React.createElement("div", {
    style: {
      gridArea: "news",
      display: "flex",
  ...
  }, /*#__PURE__*/React.createElement(SectionLabel, null, "Was gibt's Neues?"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      gap: 6
    }
  }, news.map((_, i) => /*#__PURE__*/React.createElement("div", {
    key: i,
    style: {
      width: 28,
      height: 3,
      borderRadius: 2,
      background: i === safeIdx ? "rgba(40,35,28,0.15)" : "rgba(40,35,28,0.07)",
      overflow: "hidden",
      position: "relative"
    }
  }, i === safeIdx && /*#__PURE__*/React.createElement("div", {
    style: {
      position: "absolute",
      inset: 0,
      width: `${prog * 100}%`,
  ...
  }) : /*#__PURE__*/React.createElement(PlaceholderPhoto, {
    label: item.caption || "",
    tone: "warm",
    style: {
      width: "100%",
  ...
  }, String(safeIdx + 1).padStart(2, "0"), " / ", String(news.length).padStart(2, "0")), /*#__PURE__*/React.createElement("span", {
    style: {
      flex: 1,
      height: 1,
      background: "rgba(40,35,28,0.12)"
    }
...
function ClosingBlock({
  ...
  const closeM = opening && !opening.closed && opening.closeM != null ? opening.closeM : 0;
  ...
  const isClosed = opening && opening.closed;

  // Format display time as "HH:MM — HH:MM" from parsed hours, or fall back to raw zeitraum
  let zeitraumDisplay = "- -";
  if (opening && !opening.closed) {
    if (opening.openH != null) {
      const fmt = (h, m) => `${String(h).padStart(2, "0")}:${String(m).padStart(2, "0")}`;
      zeitraumDisplay = `${fmt(opening.openH, opening.openM)} — ${fmt(opening.closeH, opening.closeM)}`;
    } else if (opening.zeitraum) {
      zeitraumDisplay = opening.zeitraum;
    }
  }
  return /*#__PURE__*/React.createElement("div", {
    style: {
    ...
    style: {
      fontFamily: "'Clash Display Variable', sans-serif",
      fontWeight: 300,
      fontSize: isClosed ? 40 : 56,
      lineHeight: 1,
      color: ACCENT,
      fontVariantNumeric: "tabular-nums"
    }
  }, isClosed ? "Heute geschlossen" : zeitraumDisplay), !isClosed && opening && opening.details && /*#__PURE__*/React.createElement("div", {
    style: {
      marginTop: 14,
      fontFamily: "'Poppins', sans-serif",
      fontSize: 15,
  ...
  }, /*#__PURE__*/React.createElement("span", null, "Noch geöffnet"), /*#__PURE__*/React.createElement("span", {
    style: {
      fontFamily: "'Clash Display Variable', sans-serif",
      fontSize: 24,
      fontWeight: 300,
  ...
  }, remH, " Std ", String(remM).padStart(2, "0"), " Min"))));
}
...
function StaffMessageBlock({
  ...
  React.useEffect(() => {
    ...
    const id = setInterval(() => setIdx(i => (i + 1) % messages.length), CONFIG.slideshowSeconds * 1000);
    return () => clearInterval(id);
  }, [messages]);
  const safeIdx = idx % messages.length;
  const item = messages[safeIdx];
  return /*#__PURE__*/React.createElement("div", {
    style: {
      gridArea: "staff",
      padding: "28px 32px",
  ...
  }, "Gruß vom Team"), messages.length > 1 && /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      gap: 5
    }
  }, messages.map((_, i) => /*#__PURE__*/React.createElement("div", {
    key: i,
    style: {
      width: 6,
      height: 6,
      borderRadius: "50%",
      background: i === safeIdx ? "rgba(245,237,227,0.75)" : "rgba(245,237,227,0.2)",
  ...
  }), /*#__PURE__*/React.createElement("span", null, item.signature))));
}
...
function App() {
  ...
  const hasStaff = staffMessages !== null && staffMessages.length > 0;
  return /*#__PURE__*/React.createElement("div", {
    style: {
      position: "fixed",
      inset: 0,
  ...
  }, /*#__PURE__*/React.createElement(LoadHero, {
    level: level
  }), /*#__PURE__*/React.createElement("div", {
    style: {
      gridArea: "meta",
  ...
  }, /*#__PURE__*/React.createElement(ClockBlock, null), /*#__PURE__*/React.createElement(WeatherBlock, {
    refreshKey: refreshKey
  })), showNewsColumn && /*#__PURE__*/React.createElement(NewsSlideshow, {
    news: news,
    seconds: CONFIG.slideshowSeconds
  })), /*#__PURE__*/React.createElement("div", {
    style: {
      ...
      gridTemplateAreas: CONFIG.showAufgussplan ? hasStaff ? `"aufguss sauna sauna staff"` : `"aufguss sauna sauna"` : hasStaff ? `"sauna sauna staff"` : `"sauna sauna"`,
      gap: 22,
      height: 300
    }
